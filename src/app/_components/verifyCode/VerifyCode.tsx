...
export default function VerifyCode({setCurrentPage}:{setCurrentPage: React.Dispatch<React.SetStateAction<string>>}) {
    ...
    const validationSchema = Yup.object({
        resetCode: Yup.string().required("ResetCode is required").matches(/^\d{6}$/,"please enter a valid reset code"),
    })
                            ...
                            <form onSubmit={formik.handleSubmit} className='flex flex-col gap-6'>
                                ...
                                <div className="inputs-Group flex flex-col gap-4">
                                    ...
                                    {formik.touched.resetCode && formik.errors.resetCode &&
                                        <p className='text-red-500 mt-1'>{formik.errors.resetCode}</p>
                                    }
                                </div>

                                <Link href={"/forgotYourPassword"} className="text-xs text-[#4461F2] text-end">Recover Password ?</Link>

                                <button type='submit' className='bg-[#4461F2] text-white p-2 rounded-[20px]'>Verify {isLoading && <i className='fas fa-spinner fa-spin'></i>}</button>
